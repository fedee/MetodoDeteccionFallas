...
class ReportePDF extends CI_Controller {
    ...
    public function verreporteconfallatipica($idcaso) {
    ...
    //Falla típica y sugerencias

    $falla = $this->casos_model->devolver_fallaporidcaso($idcaso);

    $fallahtml = '<h3>FALLA TÍPICA DE LA PIEZA</h3>
    <span style="text-align:justify;">'.$falla.'</span><br/>';

    $pdf->writeHTML($fallahtml, true, false, true, false, '');

    $sugerencia = $this->casos_model->devolver_sugerenciaporidcaso($idcaso);    

    $sugerenciahtml = '<h3>SUGERENCIA</h3>
    <span style="text-align:justify;">'.$sugerencia.'</span>';

    $pdf->writeHTML($sugerenciahtml, true, false, true, false, '');
